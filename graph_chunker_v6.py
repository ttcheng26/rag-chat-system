...
# # 目的： 判斷這一行 Markdown 表格列，是不是「表頭 (Header)」。
# # def looks_like_header(cols):
# #     for col in cols:
# #         if len(col) > 30: return False
# #         if re.search(r'\d{2,}', col) and len(cols) > 2: return False
# #         if "違反" in col or "處新臺幣" in col: return False
# #     return True
# def looks_like_header(cols):
#     # 1. 欄位太少通常不是正規表格
...
#     # --- 判定邏輯 B (年份豁免條款 - 這是您的救星！) ---
#     # 條件：
#     # 1. 必須有關鍵字 (防止數據列誤入)
#     # 2. 把「年份」扣掉後，剩下的數字欄位必須很少 (代表這一列主要由文字+年份組成)
...
#     # 讀取剛剛第一步產生的 Markdown 檔案
#     # input_md_file = "parsed_result_v6_full.md"
...
#         #     print("成功！抓到表格內容了！")
#         # else:
#         #     print("警告：還是沒有抓到 TableItem，請檢查邏輯。")
